...
function cambiarLimiteDeExtraccion() {
    var cambiarLimiteDeExtraccion = prompt(
        "Ingrese un nuevo límite de extracción:"
    );
...
function extraerDinero() {
    ...
    if (extraerDinero == null) {
        ...
        if (dineroExtraido > saldoCuenta ) {
            alert(
                "No hay saldo disponible para extraer esa cantidad de dinero."
            );
        } 
        else if (dineroExtraido > limiteExtraccion ) {
            alert(
                "La cantidad que deseas extraer es mayor a tu límite de extracción."
            );
        }
        else if (isNaN(dineroExtraido)){
            alert(
                "Solo puedes ingresar el monto."
            );
        }        
    ...
    if (servicioAPagar == null) {
        ...
        function pagoDeServicio(servicio, precioDeServicio) {
            ...
            if (saldoCuenta < precioDeServicio) {
                alert(
                    "No hay suficiente saldo para pagar este servicio."
                );
            } 
            else {
                alert(
        ...
        switch (servicioAPagar) {
            case 1:
                pagoDeServicio(
                    "Agua", precioDeServicioAgua
                );
                break;
            case 2:
                pagoDeServicio(
                    "Luz", precioDeServicioLuz
                );
                break;
            case 3:
                pagoDeServicio(
                    "Internet", precioDeServicioInternet
                );
                break;
            case 4:
                pagoDeServicio(
                    "Teléfono", precioDeServicioTelefono
                );
                break;            
            default:
                alert(
                    "No existe el servicio que se ha seleccionado."
                );
                break;
        }
    }
...
function transferirDinero() {
    var montoATransferir = prompt(
        "Ingrese el monto que desea transferir:"
    );

    if(saldoCuenta < montoATransferir) {
        alert(
            "No puede transferirse esa cantidad de dinero."
        );
    }
    else if (montoATransferir == null) {
        alert(
            "No ingreso monto para la transferencia."
        );
    }
    else if (isNaN(montoATransferir)) {        
        alert(
            "Ingrese solo el monto."
        );
    }
    else{
        ...
        if (cuentaATransferir == null) {
            alert(
                "No ingreso cuenta para la transferencia."
            );
        }
            ...
            switch (cuentaATransferir) {
                ...
                default:
                    alert(
                        "Solo puede transferirse dinero a una cuenta amiga."
                    );
                    break;
            }
        }
...
function iniciarSesion() {
    ...
    if (codigoVerificacion == null) {
        alertaCodigoIncorrecto(
            "Usted no ingreso un código de verficación."
        );
    } 
    else {
        ...
        if (codigoVerificacion == codigoCuenta) {
            alert(
                "Bienvenido/a " + nombreUsuario + " ya puedes comenzar a realizar operaciones."
            );
        } 
        else {
            alertaCodigoIncorrecto(
